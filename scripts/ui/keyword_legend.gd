...
func _build_legend() -> void:
	...
	_add_keyword("Slowed", "Movement costs 3 tempo per tile instead of 1; each tile moved burns a stack", Color(0.3, 0.3, 0.6))
	_add_keyword("Staggered", "Attack cards cost 15 more mana; each attack card burns a stack", Color(0.6, 0.4, 0.2))
	_add_keyword("Drain", "Lose 10 mana per cycle, lose 1 drain per cycle", Color(0.4, 0.0, 0.6))
	_add_keyword("Weighted", "Cards cost 2 more tempo; each card played burns a stack", Color(0.4, 0.4, 0.4))
	...
	_add_keyword("Cold", "Stacking debuff. At 5 stacks the target becomes Frozen for 1 cycle", Color(0.4, 0.7, 1.0))
	...
	_add_keyword("Engrave", "Cannot join your deck at all unless slotted into an item", Color(0.75, 0.65, 0.4))
	...
	_add_keyword("Overdraw", "Equipment riders that fire whenever a draw overflows a full hand — they pay out alongside whichever overflow mode is set")
	...
	_add_keyword("Conditional", "Takes the reach of the weapon in hand: melee with a melee weapon (sword, axe, dagger, hammer, spear) or bare hands; Ranged 5 with a bow. The basic Attack card is also Ranged 5 with a magic weapon (wand, tome, staff). Costs +1 tempo while it plays as ranged", Color(0.6, 0.8, 0.7))
