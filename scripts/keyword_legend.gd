...
func _build_legend() -> void:
	...
	_add_keyword("Burn", "Burn damage doubles each turn (1, 2, 4, 8...)", Color(1.0, 0.5, 0.0))
	_add_keyword("Poison", "Take X damage per turn, lose 1 poison each turn", Color(0.2, 0.8, 0.2))
	...
	_add_keyword("Shocked", "Deal X damage to nearby allies per turn, lose 1 per turn", Color(1.0, 1.0, 0.3))
	...
	_add_keyword("Drain", "Lose 1 mana per turn, lose 1 drain per turn", Color(0.4, 0.0, 0.6))
	...
	_add_keyword("Magnetized", "Pulled toward nearest enemy at end of each turn", Color(0.2, 0.2, 0.8))
	...
	_add_keyword("Vulnerable", "Take 30% more damage on next attack, lose 1 stack per hit", Color(1.0, 0.3, 0.3))
	_add_keyword("Exposed", "Triggered when an enemy's armor is reduced to zero", Color(0.9, 0.7, 0.5))
	_add_keyword("Brittle", "Armor decays extra 2 per turn, lose 1 stack per turn", Color(0.7, 0.7, 0.6))
	...
	_add_keyword("Regen", "Heal X HP per turn, lose 1 regen per turn", Color(0.4, 1.0, 0.4))
	...
	_add_keyword("Brace", "Reduce incoming attack damage by 30% for X attacks", Color(0.5, 0.5, 0.8))
	_add_keyword("Resilient", "Reduce all incoming damage by 15% for X turns", Color(0.7, 0.6, 0.9))
	...
	_add_keyword("Wear Down", "Each attack reduces target's attack for X turns", Color(0.9, 0.6, 0.3))
