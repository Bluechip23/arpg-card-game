...
static func create_brad_tree(max_level: int = 20) -> SkillTreeData:
	...
	var ability_placements := [
		...
		{level = 3, slot = 1, archetype = "Warden", name = "In the Trenches",
			description = "When an enemy enters an adjacent square, perform a free attack at 50%→120% damage (scales with rank). When an enemy attacks you from adjacent, knock them back. 2 charges, 10 tempo cooldown",
		...
		{level = 9, slot = 3, archetype = "The Ancient", name = "Ancestral Aid",
			description = "Every 5 cycles: more attacks in hand → discount a random attack in your hand by 50m→190m for 5 tempo. More defense → heal 3→17 HP (scales with rank)",
			color = Color(0.4, 0.9, 0.4)},
		...
		{level = 16, slot = 1, archetype = "The Ancient", name = "Vines Codependence",
			description = "Whenever you heal (direct heals only — regen and life steal don't count), gain 1→8 thorns and 0→7 regen for 5 tempo (scales with rank)",
			color = Color(0.4, 0.9, 0.4)},
...
static func create_stephen_tree(max_level: int = 20) -> SkillTreeData:
	...
	var ability_placements := [
		...
		{level = 5, slot = 3, archetype = "Avenger", name = "Patience is a Virtue",
			description = "When receiving Glut, deal 10%→290% of the Glut amount (scales with rank) as damage to an enemy in melee range, then halve the Glut",
			color = Color(0.8, 0.4, 0.9)},
		{level = 7, slot = 1, archetype = "The Apex", name = "Easy Target",
			description = "When exposing your enemy, deal your damage again and gain +1→15 Strengthen for 3 attacks (scales with rank)",
			color = Color(0.9, 0.3, 0.3)},
		{level = 8, slot = 0, archetype = "Sentinel", name = "Exposed Blind Spot",
			description = "When struck with a melee attack, gain crit chance on your next attack equal to 1%→4.5% (scales with rank) per non-attack card in your hand",
			color = Color(0.3, 0.7, 1.0)},
		{level = 9, slot = 3, archetype = "Ranger", name = "Scouted",
			description = "Hitting the same enemy 3 times in a row grants +2→6 range on your next attack and it auto-crits with +2%→30% crit damage (scales with rank) — usable against any enemy",
			color = Color(0.4, 0.9, 0.4)},
		{level = 11, slot = 2, archetype = "Avenger", name = "Swing for the Fences",
			description = "Cards that have >4 tempo cost deal 100%→380% of their tempo cost (scales with rank) as additional damage",
			color = Color(0.8, 0.4, 0.9)},
		{level = 13, slot = 0, archetype = "The Apex", name = "Skilled Momentum",
			description = "If you have played 10→3 attacks in a row (scales with rank), your next will be played twice. Cooldown 5 tempo",
			color = Color(0.9, 0.3, 0.3)},
		{level = 14, slot = 2, archetype = "Sentinel", name = "Lethal Resourcefulness",
			description = "If you have 3 or less cards in your hand, playing a non-attack card triggers a free basic attack. Cooldown 40→12 tempo (scales with rank)",
			color = Color(0.3, 0.7, 1.0)},
		{level = 16, slot = 1, archetype = "Ranger", name = "Laced Arrow",
			description = "When applying burn, cold, or shock, you have a 50%→100% chance (scales with rank) to apply 1 additional charge",
			color = Color(0.4, 0.9, 0.4)},
		{level = 18, slot = 3, archetype = "Avenger", name = "Dominate",
			description = "When triggering an attack speed proc, the proc resolves normally and you also gain a 30m/0t basic attack card and +2→16 Strengthen for 3 attacks (scales with rank). Cooldown 5 tempo",
			color = Color(0.8, 0.4, 0.9)},
	]
...
static func create_ryan_tree(max_level: int = 20) -> SkillTreeData:
	...
	var ability_placements := [
		{level = 2, slot = 0, archetype = "Relentless Blade", name = "Keep Them Guessing",
			description = "After discarding 18→4 cards (scales with rank), -3t from a random card in your hand for 5 tempo",
			color = Color(0.9, 0.3, 0.3)},
		{level = 3, slot = 1, archetype = "Light Foot", name = "Quick Step",
			description = "Anytime an instant is played from your hand, gain 2→16 armor (scales with rank)",
			color = Color(0.3, 0.7, 1.0)},
		{level = 4, slot = 2, archetype = "Apothecary", name = "Stimulant",
			description = "When healing yourself or an ally with a Pocket card, the healed character draws a card. Cooldown 19→5 tempo (scales with rank).",
			color = Color(0.4, 0.9, 0.4)},
		{level = 5, slot = 3, archetype = "Shadow Blade", name = "Now You See Me",
			description = "Displacing yourself (blinking, being swapped, or other non-standard movement) grants you invisibility for 5 tempo. Cooldown 15→1 tempo (scales with rank)",
			color = Color(0.8, 0.4, 0.9)},
		{level = 7, slot = 1, archetype = "Relentless Blade", name = "From the Hip",
			description = "If an attack, your most recently drawn card has -10m→-75m mana cost (scales with rank; ranks 11+ also -1 to -2 tempo). The discount is lost when any card is played",
			color = Color(0.9, 0.3, 0.3)},
		{level = 8, slot = 0, archetype = "Light Foot", name = "Ladder Work",
			description = "+1→8 dexterity and +1→15 agility (scales with rank). Your first attack each cycle deals +1→6 damage for each card that hit your discard pile last cycle by means other than playing it",
			color = Color(0.3, 0.7, 1.0)},
		{level = 9, slot = 3, archetype = "Apothecary", name = "Pop Rocks",
			description = "When applying poison to an already poisoned enemy, deal immediate damage equal to 1/3 their current poison stacks. Cooldown 19→5 tempo (scales with rank)",
			color = Color(0.4, 0.9, 0.4)},
		{level = 11, slot = 2, archetype = "Shadow Blade", name = "Surprise Opener",
			description = "Your first strike on an enemy deals +1→8 damage. An additional +1→15 if they have no armor, and an additional +3→17 if you are their first source of damage (scales with rank)",
			color = Color(0.8, 0.4, 0.9)},
		{level = 13, slot = 0, archetype = "Relentless Blade", name = "Nimble Assault",
			description = "If you have cards in your hand, but no Defense cards, draw a card when you play an attack. Cooldown 15→8 tempo (scales with rank)",
			color = Color(0.9, 0.3, 0.3)},
		...
		{level = 16, slot = 1, archetype = "Apothecary", name = "Mad Scientist",
			description = "The last card you played changes the outcome of your potion cards (scales with rank). Utility→Heal: +2→8 regen. Attack→Heal: +2→8 strengthen for 3 attacks. Utility→Poison: +2→9 poison stacks. Defense→Poison: the enemy loses 1%→15% of its current armor.",
			color = Color(0.4, 0.9, 0.4)},
		{level = 18, slot = 3, archetype = "Shadow Blade", name = "Eye Scrape",
			description = "Every 15th→1st critical strike (scales with rank) provides invisibility for 5 tempo. Cooldown 10 tempo",
			color = Color(0.8, 0.4, 0.9)},
	]
...
static func create_cory_tree(max_level: int = 20) -> SkillTreeData:
	...
	var ability_placements := [
		...
		{level = 16, slot = 1, archetype = "Lurker", name = "Serial Killer",
			description = "The first time an enemy drops below 11%→25% health (scales with rank), you become invisible to them until you attack them. That attack, out of your invisibility, is an auto-crit",
			color = Color(0.3, 0.7, 1.0)},
...
static func create_jeremy_tree(max_level: int = 20) -> SkillTreeData:
	...
	var ability_placements := [
		{level = 2, slot = 0, archetype = "Evocation", name = "Arcane Overflow",
			description = "When you have 0 mana remaining after casting a spell, your next spell costs -1 tempo. Cooldown 20→6 tempo (scales with rank)",
			color = Color(0.9, 0.3, 0.3)},
		{level = 3, slot = 1, archetype = "Shepherd", name = "I Heal You",
			description = "Allies within 3 squares of you are healed 3 health every 18→4 tempo (scales with rank)",
			color = Color(0.3, 0.7, 1.0)},
		{level = 4, slot = 2, archetype = "Poltergeist", name = "Tricks of Death",
			description = "Increase all % chances on your CARDS by 5→12 (scales with rank; item procs like the Stringless Sender's bounce are unaffected)",
			color = Color(0.4, 0.9, 0.4)},
		...
		{level = 9, slot = 3, archetype = "Poltergeist", name = "Seance",
			description = "When you cast a spell that targets an empty tile, summon a Specter on that tile for 25 tempo. 5→33 HP (scales with rank), cannot act. Enemies may target it based on proximity. When destroyed, deals its HP as damage to its killer.",
			color = Color(0.4, 0.9, 0.4)},
		{level = 11, slot = 2, archetype = "Abjurer", name = "Kinetic Armor",
			description = "While retaining armor for more than 30→16 tempo (scales with rank), apply shock to the nearest enemy equal to the number of defensive cards in your deck",
			color = Color(0.8, 0.4, 0.9)},
		{level = 13, slot = 0, archetype = "Evocation", name = "Mana Surge",
			description = "When spending 100 mana within 5 tempo, add a Mana Surge to your hand. Mana Surge: Attack, 0m/2t, deal 4→18 damage (scales with rank), gain 10 mana.",
			color = Color(0.9, 0.3, 0.3)},
		{level = 14, slot = 2, archetype = "Poltergeist", name = "Haunted Rebuke",
			description = "When an enemy attacks you and you have 3 or more defensive cards in your hand, the enemy's next action takes +3 tempo. Cooldown: 25→11 tempo (scales with rank).",
			color = Color(0.4, 0.9, 0.4)},
		{level = 16, slot = 1, archetype = "Shepherd", name = "Blood Libation",
			description = "Every time Jeremy takes damage (excluding Blood Libation's own self-damage), gain a stack of Sanguine (max 5). Each stack grants +1→15 (scales with rank) to all healing Jeremy performs. At 5 stacks, Jeremy's next heal is doubled, all Sanguine stacks are consumed, and Jeremy takes 10 non-lethal damage. The damage resolves FIRST, then the heal",
			color = Color(0.3, 0.7, 1.0)},
		{level = 18, slot = 3, archetype = "Abjurer", name = "Fresh Start",
			description = "When playing a card that leaves your hand empty, cleanse a debuff from yourself. Cooldown 25→11 tempo (scales with rank)",
			color = Color(0.8, 0.4, 0.9)},
	]
