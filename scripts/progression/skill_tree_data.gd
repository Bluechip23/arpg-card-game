...
static func create_ryan_tree(max_level: int = 20) -> SkillTreeData:
	...
	var ability_placements := [
		...
		{level = 16, slot = 1, archetype = "Apothecary", name = "Mad Scientist",
			description = "The last card you played changes the outcome of your potion cards (scales with rank). Utility→Heal: +2→8 regen. Attack→Heal: +2→8 strengthen for 3 attacks. Utility→Poison: +2→9 poison stacks. Defense→Poison: -1%→-15% enemy physical defense for 5 tempo.",
			color = Color(0.4, 0.9, 0.4)},
