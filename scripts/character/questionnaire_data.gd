...
## The final character gets custom stats, the same 11-card basic deck as every
## preset (4 slash, 4 block, draw, energy, heal — the deck cap is 12), and an
...
## The passive and slot specialty come from one existing character.
static func compute_result(answer_indices: Array[int]) -> Dictionary:
	...
	# Quiz characters share the identical basic deck with everyone else — no
	# extra starting cards.
	var starting_card_ids: Array = []

	# Get existing passive/slot data from the primary archetype's character
