...
| A3 | **[hand-verified]** Arcane Current permanently mutates the card: `bonus_damage += 5` per cast with no removal — compounds without bound | `progression_triggers.gd:2002` | open — deferred until constellation work |
...
| A5 | **[hand-verified]** Growth Within Resilience never triggers in solo play (its hook lives in `_on_ally_damage_taken`, connected only in co-op/sandbox); also reads the active player's stats, not the victim's | `main.gd:9072-9078,569-572` | fixed — card deleted entirely (factory, rarity table, Hydra drop, maintain hook, sandbox list) |
...
| # | Finding | Where | Status |
|---|---|---|---|
| B1 | Consecutive Snap resolves one use ahead under deferred execution: first play deals 12 (3+9) and discounts the next use immediately | `deck_manager.gd:551`, `card.gd:2125` | fixed — use count snapshotted at play time |
| B2 | Burden mechanic entirely unimplemented — helpers have no callers; Healthy Habit & Provider advertise it | `card.gd:1280-1295,4430,4972` | fixed — cost escalates +1m/+1t per play; J key jails from hand (1m/1t, 30-tempo jail resets it) |
| B3 | Blade Barrage advertises "Glut: 15 tempo" but never sets `glut_tempo` | `card.gd:3338-3353` | fixed — glut_tempo = 15 |
...
| B5 | Surrounding Ice: chance is pre-rolled per enemy but the live path re-rolls a hard-coded 70%; `get_rng_outcome` has zero callers, the RNG preview indicator is an empty stub, so Loaded Die/House Money can't affect it | `card.gd:241-248`, `main.gd:7300`, `aoe_indicator.gd:117` | fixed — live path reads the pre-rolled boost-aware outcomes; late-spawned enemies roll lazily at the same boosted chance |
...
| # | Finding | Where | Status |
|---|---|---|---|
| C1 | Flash Cut ignores the free-hand parry discount (hard-codes cost 3; Sidestep correctly pays 2) | `player_stats.gd:829-831`, `main.gd:4759` | fixed — Flash Cut pays 2 with a free hand |
...
| C9 | Skill-tree passives with no source: Sword Specialist, Phalanx, Corrupted Strength, Disarm Mastery handlers exist but no tree option produces those ids | `progression_triggers.gd:573,1269,1056,1306` | fixed — added as tree options: Brad Lv6 (Corrupted Strength), Lv10 (Phalanx), Lv12 (Sword Specialist); Cory Lv10 (Disarm Mastery) |
