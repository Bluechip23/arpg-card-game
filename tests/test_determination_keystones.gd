...
##   Unbroken Will (det_floor)  — raises the penalty clamp from 0.1 to 0.5
...
func _test_modifier_math() -> void:
	...
	var data = CharacterData.create_ryan()  # DET 3 (det_diff = -7)
	var stats = PlayerStats.new()
	get_root().add_child(stats)
	stats.initialize(data)
	# Drive health to <=10% so the strongest band (0.10 per point) applies.
	stats.current_health = maxi(1, int(stats.max_health * 0.1))

	# Baseline: 1.0 + (-7 * 0.10) = 0.30
	_check(_approx(stats.get_determination_modifier(), 0.30),
		"baseline penalty at 10%% HP, DET 3 = 0.30 (got %.2f)" % stats.get_determination_modifier())

	# Unbroken Will: penalty clamped up to 0.50
	...
	# Wild Abandon: swing ×1.5 -> 1.0 + (-1.05) clamps at 0.10
	stats.keystone_det_amplify = true
	_check(_approx(stats.get_determination_modifier(), 0.10),
		"Wild Abandon deepens the penalty to the 0.10 floor (got %.2f)" % stats.get_determination_modifier())
	...
	stats.determination = 20  # det_diff = +10 -> 1.0 + 1.0 = 2.0 baseline
	_check(_approx(stats.get_determination_modifier(), 2.0),
		"high-DET buff baseline = 2.0 (got %.2f)" % stats.get_determination_modifier())
	stats.keystone_det_amplify = true
	_check(_approx(stats.get_determination_modifier(), 2.5),
		"Wild Abandon raises the buff to 2.5 (got %.2f)" % stats.get_determination_modifier())
