...
func _initialize() -> void:
	...
	# --- Set up stats + inventory the way Player does (Brad: 3 hand slots) ---
	...
	# The locked hand refuses items; Brad's third hand still works.
	var shield := ItemData.create_wooden_shield()
	_check(not inv.equip_item(shield, 1), "locked hand slot refuses the shield")
	_check(inv.equip_item(shield, 2), "third hand slot still accepts the shield")
	...
	_check(inv.get_total_weight() == 134, "weights restore (130 + 4)")
	...
	_check(inv.get_two_hand_block_bonus(tower) == 20, "braced shield: +20 Basic Block armor (60/3)")
	...
	# Current state: greatsword slot 0 (one-handed), shield slot 2.
	...
	_check(inv.unequip_to_storage(ItemData.ItemType.WEAPON, 2), "build II: shield unequips to storage")
	...
	_check(inv.get_equipped_item(ItemData.ItemType.WEAPON, 2) == shield, "build I re-equips the shield")
	...
	_check(inv.get_equipped_item(ItemData.ItemType.WEAPON, 2) == null, "build II sheds the shield")
