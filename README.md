...
**Every mythic has a face.** A mythic carries an `appearance` — a description of what the thing actually looks like — and a 32×32 sprite drawn from that description (`assets/items/mythic/`, generated by `tools/generate_mythic_icons.py`). Equip it and the inventory drops the generic slot silhouette for the item's own art; the item detail panel shows the art blown up with the description under it. Ordinary gear keeps the silhouette.
